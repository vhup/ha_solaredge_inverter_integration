...
from .const import CONF_SITE_ID, CONF_INVERTER_ID, DEFAULT_NAME, DOMAIN
...
class SolarEdgeConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _check_site(self, site_id: str, inverter_id:str, api_key: str) -> bool:
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if self._site_in_configuration_exists(user_input[CONF_SITE_ID]):
                ...
                inverter = user_input[CONF_INVERTER_ID]
                api = user_input[CONF_API_KEY]
                """we should probably also add a check on the inverter id"""
                can_connect = await self.hass.async_add_executor_job(
                    self._check_site, site, inverter, api
                )
                if can_connect:
                    return self.async_create_entry(
                        title=name, data={CONF_SITE_ID: site, CONF_INVERTER_ID: inverter,CONF_API_KEY: api}
                    )

        else:
            user_input = {CONF_NAME: DEFAULT_NAME, CONF_SITE_ID: "", CONF_INVERTER_ID: "", CONF_API_KEY: ""}
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_INVERTER_ID, default=user_input[CONF_INVERTER_ID]): str,
